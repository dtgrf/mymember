...
public class VerificationCodeServiceImpl extends BaseServiceImpl<VerificationCodeMapper, VerificationCode> implements VerificationCodeService {
    ...
    public EmailVo sendEmail(VerificationCode code) {
        ...
        if (verificationCode == null) {
            ...
            emailVo = new EmailVo(Collections.singletonList(code.getValue()), "member后台管理系统", content);
            this.save(code);
            timedDestruction(code);
            // 存在就再次发送原来的验证码
        } else {
            content = template.render(Dict.create().set("code", verificationCode.getCode()));
            emailVo = new EmailVo(Collections.singletonList(verificationCode.getValue()), "member后台管理系统", content);
        }
        return emailVo;
    }
    ...
    public void validated(VerificationCode code) {
        ...
        if (verificationCode == null || !verificationCode.getCode().equals(code.getCode())) {
            ...
            LambdaUpdateWrapper<VerificationCode> lambdaUpdateWrapper = new LambdaUpdateWrapper<>();
            lambdaUpdateWrapper.eq(VerificationCode::getId, verificationCode.getId()).
                    set(VerificationCode::getStatus, verificationCode.getStatus());
