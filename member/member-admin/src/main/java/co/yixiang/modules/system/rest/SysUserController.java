...
public class SysUserController {
    ...
    @PostMapping(value = "/UpUserAndAvatar")
    @PreAuthorize("@el.check('admin','user:add')")
    public ResponseEntity<Object> Managercreate(@Validated @RequestBody User resources) {

        checkLevel(resources);
        // 默认密码 123456
        resources.setPassword(passwordEncoder.encode("123456"));
        return new ResponseEntity<>(userService.create1(resources), HttpStatus.CREATED);
    }
    @ForbidSubmit
    @ApiOperation("上传头像")
    @PostMapping(value = "/preUpdateAvatar")
    public String PreupdateAvatar(@RequestParam MultipartFile file) {
        String id=userService.PreUpdateAvatar(file);
        return id;
    }
